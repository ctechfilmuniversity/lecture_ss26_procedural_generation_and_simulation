...
_Please note the we haven't covered all topics in slides and script yet._

### Task 02.01 - Inspiration - 4 Points

Go to the [shadertoy](https://www.shadertoy.com/browse) site and browse the examples a bit. Submit the link to at least one example you like (you don't have to understand the code). Think about _what_ you like about the example and _why_. You don't have to write anything about that in your submission but be able to explain it in class.

_On a side note_: shadertoy code does not directly run within the glsl-canvas environment (see Task 2.2).
...
![pattern02](img/screen01.png)

![pattern02](img/screen02.png)

![pattern02](img/screen03.png)

![pattern02](img/screen04.png)
...
\_circles_nodes` 2. Function design with code (HLSL) in`M_pattern_circles_hlsl` .
...
My learnings were that I wanted to make geometrically structured patterns turn dynamic and unsymmetrical again.
...
coord /= CELLSIZE;
Dividing by 0.2 scales up the coordinate space — effectively this creates repeated "cells" of size 0.2 in the original space (because now what used to span 0.2 units spans 1.0 unit). Note: this alone doesn't wrap/repeat (no fract or mod is used)

coord -= sin(coord);

This is the key "trick" line, and it's doing something subtler than simple tiling. sin(coord) is periodic, so subtracting it introduces a periodic warp into the coordinate field rather than a hard repeat. Because sin is bounded between -1 and 1 while coord grows unboundedly, this creates a pattern where:

Near coord = 0, sin(coord) ≈ coord, so the result is close to 0 (cancels out).

Farther out, the linear growth of coord dominates over the bounded sin(coord), so the result grows roughly linearly again, but with a wavy/oscillating perturbation layered on top.

This produces smoothly recurring "lobes" or wave-like bands rather than sharp cell boundaries — it's a soft, organic-looking alternative to mod()-based tiling.

This phenomena I found very fascinating, and I want to use it in further shader examples
